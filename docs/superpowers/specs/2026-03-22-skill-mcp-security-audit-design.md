...
**SKILL.md mismatch check:** If a `SKILL.md` companion file exists and declares read-only/no-network capabilities but the code contains network or write patterns, this is flagged as a mismatch (separate penalty in scorer).
...
Trust bonus:    +10 if source path contains known-good prefix
                    (anthropic-skills, huggingface/official, configurable list)
Floor: 0  Ceiling: 100
```
...
Daily summary: `~/openclaw/security/audits/summary-<YYYY-MM-DD>.md` — table of all skills, scores, categories, last verified.
...
3. **Remote flag**: if npm package or remote URL → flag for manual review, score 60 (REVIEW)
...
- Hash verification runs on every load, not just install
