...
- **Phase 2 — Alive (speed & presence)** — ✦ SHIPPED 2026-06-09 (`df23caf` client-mcp · `7481027` openclaw)
  - [x] Streaming naming: `llm.ts` onToken (Ollama NDJSON) → `POST /v1/demo/naming-stream` (start/token/result events) → portal live "wire" view + parsed cards; dashboard proxy passes `*-stream` through chunked (token arrival timestamped at :4000 and :7080). Warm model returns 8 candidates in ~2-3s. Edge finding: Cloudflare buffers x-ndjson → reframed as SSE (text/event-stream), edge-verified live via GET /v1/demo/ping-stream (ticks 700ms apart through cloudflared). NOTE: per-IP rate bucket sees the proxy's 127.0.0.1 for all public users (shared bucket) — fwd X-Forwarded-For + trustProxy when it matters.
